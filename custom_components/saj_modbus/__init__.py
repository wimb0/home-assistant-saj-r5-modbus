...
from .inverter import params_from_config
...
async def async_migrate_entry(hass: HomeAssistant, entry: SajConfigEntry) -> bool:
...
async def async_setup_entry(hass: HomeAssistant, entry: SajConfigEntry) -> bool:
    ...
    # One construction path for both transports; serial line settings are
    # fixed to 8N1 inside params_from_config, historical entries default TCP.
    hub = SAJModbusHub(
        hass,
        entry,
        name,
