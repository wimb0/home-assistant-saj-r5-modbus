"""Tests for tolerating blocks the inverter does not serve."""
...
def block_error(code: int) -> BlockReadError:
    """Build a block rejection carrying ``code``."""
    return BlockReadError("holding", 0x1037, 1, code)


@pytest.mark.parametrize("code", [1, 2])
def test_structural_rejection_marks_block_absent(code: int) -> None:
    """Illegal function and illegal data address mean the block is not there."""
    hub = make_hub()

    hub._note_absent("power", block_error(code))
